...
\begin{frame}{Table 3: Interpretation of Results}

\begin{itemize}
\item \textbf{Table 3} presents the statistical errors of the neural network across different option contracts
\item The accuracy varies significantly between contracts
\item Options with lower strike prices (in-the-money) generally show lower errors
\item Options with higher errors tend to be less stable or less frequently traded
\end{itemize}

\vspace{0.3cm}

\textbf{Key insight:} The performance of the neural network depends on the characteristics of the option, particularly trading activity and moneyness.
\end{frame}

\begin{frame}{Figure 3: Model Comparison}
\begin{itemize}
\item \textbf{Figure 3} compares:
    \begin{itemize}
        \item Market price (SPOT)
        \item Black--Scholes (BLS)
        \item Neural network (NN)
    \end{itemize}
\item (a): Best NN prediction (low MSE)
\item (b): Worst NN prediction (high MSE)
\item NN follows market trend well in (a), but deviates more in (b)
\end{itemize}

\vspace{0.3cm}

\textbf{Key insight:} Neural networks can closely match market prices in some cases, but performance is not consistent across all scenarios.
...
For example, an Asian option depends on the average stock price over time:
\[
...
\item For the classical Black--Scholes model, closed-form or fast numerical methods are often more efficient
\end{itemize}
